...
const DonarSignUp = () => {
    ...
    const signupDonar = async (e) => {
        e.preventDefault();
        let { name, contact, email, password } = data;
        email = email.split(" ").join("");
        try {
            const { data } = await axios.post("/signupDonar", {
                name, 
                contact,
                email,
                password,
                    ...
                    <div className="col-md-6 right-box">
                        <div className="row align-items-center">
                            <div className="header-text mb-4 mt-5 d-flex flex-column justify-content-center align-items-center">
                                ...
                                <p style={{ fontFamily: 'Poppins', fontWeight: 400 }}>Donate Blood. Signup to continue.</p>
                            </div>
                            ...
                            <div className="input-group mb-3">
                                <button className="btn btn-lg btn-primary w-100 fs-6" onClick={signupDonar}>Signup</button>
                            </div>
                        </div>
                    </div>
                </div>
